...
class Visualization(object):
    def __init__(self, args):
        ...
        # self.dataloader = torch.utils.data.DataLoader(dataset, batch_size=1, shuffle=True, num_workers=0)
        self.dataloader = SonarDataLoader(args).load_data()
        self.model = SONICModel(args)
    ...
    def plot_correspondence(self):
        ...
        corr_point = self.ax.scatter(point2[0], point2[1], color=self.color)
        if self.with_std:
